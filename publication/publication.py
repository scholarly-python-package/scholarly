...
class Publication(object):
    ...
    def __init__(self, __data, scholarly, pubtype=None):
        ...
        if self.source == 'citations':
            self.citation_pub(__data)
        elif self.source == 'scholar':
            self.scholar_pub(__data)
            
        self._filled = False


    def citation_pub(self, __data):
        self.bib['title'] = __data.find('a', class_='gsc_a_at').text
        self.id_citations = re.findall(self._scholarly.URLS('CITATIONPUBRE'), __data.find('a', class_='gsc_a_at')['data-href'])[0]
    ...
    def scholar_pub(self, __data):
        ...
        for link in lowerlinks:
            ...
            if 'Cited by' in link.text:
                ...
                self.bib['citesurl'] = self._scholarly.URLS('HOST').format(link['href'])
    ...
    def get_bibtex(self, cid:str, pos:str) -> str:
    ...
    def fill(self):
        ...
        if self.source == 'citations':
            ...
            soup = self._scholarly._get_soup(self._scholarly.URLS('HOST').format(url))
            ...
            for item in soup.find_all('div', class_='gs_scl'):
                ...
                if key == 'Authors':
                    ...
                    self.id_scholarcitedby = re.findall(self._scholarly.URLS('SCHOLARPUBRE'), val.a['href'])[0]
    ...
    def get_citedby(self):
        ...
        if hasattr(self, 'id_scholarcitedby'):
            url = self._scholarly.URLS('SCHOLARPUB').format(requests.utils.quote(self.id_scholarcitedby))
            soup = self._scholarly._get_soup(self._scholarly.URLS('HOST').format(url))
