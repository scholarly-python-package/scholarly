...
from .data_types import Author, AuthorSource, PublicAccess, PublicationSource
...
class AuthorParser:
    ...
    def get_author(self, __data) -> Author:
        ...
        if isinstance(__data, str):
            ...
            author["scholar_id"] = re.findall(_CITATIONAUTHRE, __data("a")[0]["href"])[
                0
            ]
    ...
    def _fill_basics(self, soup, author):
        ...
        author["interests"] = [
            i.text.strip() for i in soup.find_all("a", class_="gsc_prf_inta")
        ]
    ...
    def _fill_public_access(self, soup, author):
        ...
        author["public_access"] = PublicAccess(
            available=n_available, not_available=n_not_available
        )
        ...
        while True:
            ...
            if rows:
                for row in rows[0].find_all("a", "gsc_mnd_art_rvw gsc_mnd_link_font"):
                    author_pub_id = re.findall(
                        r"citation_for_view=([\w:-]*)", row["data-href"]
                    )[0]
    ...
    def _fill_publications(
        self, soup, author, publication_limit: int = 0, sortby_str: str = ""
    ):
        ...
        while True:
            for row in soup.find_all("tr", class_="gsc_a_tr"):
                ...
                if (publication_limit) and (
                    len(author["publications"]) >= publication_limit
                ):
                    flag = True
                    break
            if (
                "disabled" not in soup.find("button", id="gsc_bpf_more").attrs
                and not flag
            ):
                pubstart += _PAGESIZE
                url = f"{url_citations}&cstart={pubstart}&pagesize={_PAGESIZE}"
                soup = self.nav._get_soup(url)
            else:
                break
    ...
    def _get_coauthors_short(self, soup):
        ...
        coauthor_ids = [
            re.findall(_CITATIONAUTHRE, coauth("a")[0].get("href"))[0]
            for coauth in coauthors
        ]

        coauthor_names = [coauth.find(tabindex="-1").text for coauth in coauthors]
        coauthor_affils = [
            coauth.find(class_="gsc_rsb_a_ext").text for coauth in coauthors
        ]
    ...
    def _get_coauthors_long(self, author):
        ...
        coauthor_affils = [coauth.find(class_="gs_ai_aff").text for coauth in coauthors]
    ...
    def fill(
        ...
        try:
            ...
            sections.sort(
                reverse=True
            )  # Ensure 'publications' comes before 'public_access'
            sortby_str = ""
            if sortby == "year":
                sortby_str = "&view_op=list_works&sortby=pubdate"
            elif sortby != "citedby":
                raise Exception(
                    "Please enter a valid sortby parameter. Options: 'year', 'citedby'"
                )
            url_citations = _CITATIONAUTH.format(author["scholar_id"])
            url_citations += sortby_str
            ...
            # Update scholar_id
            scholar_id = re.findall(
                _CITATIONAUTHRE, soup.find("link", rel="canonical").get("href", "")
            )[0]
            if scholar_id != author["scholar_id"]:
                self.nav.logger.warning(
                    "Changing the scholar_id following redirect from %s to %s. "
                ...
                for i in self._sections:
                    if i not in author["filled"]:
                        (
                            ...
                            if i != "publications"
                            else getattr(self, f"_fill_{i}")(
                                soup, author, publication_limit, sortby_str
                            )
                        )
