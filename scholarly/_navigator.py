...
class Navigator(object, metaclass=Singleton):
    ...
    def use_freeproxy(self):
        self.pm = ProxyGenerator()
        self.pm.set_new_freeproxy()

    def use_lum_proxy(self, usr = None , passwd = None, proxy_port = None ):
        self.pm = ProxyGenerator()
        self.pm._use_lum_proxy(usr, passwd, proxy_port)

    def launch_tor(self, tor_cmd: int, tor_sock_port: int, tor_control_port:int):
        self.pm = ProxyGenerator()
        self.pm._launch_tor(tor_cmd, tor_sock_port, tor_control_port)

    def refresh_tor(self, tor_sock_port: int, tor_control_port: int, tor_password: str):
        self.pm = ProxyGenerator()
        self.pm._setup_tor(tor_sock_port = tor_sock_port, tor_control_port=tor_control_port,tor_password = tor_password)
