...
from .data_types import Author, ProxyMode, PublicationSource
from .publication_parser import PublicationParser, _SearchScholarIterator


class Singleton(type):
    """A single search instance."""
...
class Navigator(metaclass=Singleton):
        ...
        while tries < self._max_retries:
            ...
            try:
                session, timeout = pm.get_next_proxy(
                    num_tries=tries,
                    old_timeout=timeout,
                    old_proxy=session.proxies.get("http", None),
                )
            except Exception:
                self.logger.info(
    ...
    def _set_retries(self, num_retries: int) -> None:
        """Sets the number of retries allowed per search."""
        if num_retries < 0:
    ...
    def search_publication(self, url: str, filled: bool = False) -> PublicationParser:
    ...
    def search_organization(self, url: str, fromauthor: bool) -> list:
        ...
        If no results are found and `fromuthor` is True, then use the first author from the search to get institution/organization name.
        """
        soup = self._get_soup(url)
