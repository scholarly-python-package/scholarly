...
def remap_bib(
    parsed_bib: dict, mapping: dict, data_types: Optional[dict] = None
) -> BibEntry:
    ...
    for key, value in data_types.items():
        if key in parsed_bib and value == "int":
...
class _SearchScholarIterator:
    ...
    def _load_url(self, url: str):
        ...
        self._pos = 0
        self._rows = (
            self._soup.find_all("div", class_="gs_r gs_or gs_scl")
...
class PublicationParser:
    ...
    def fill(self, publication: Publication) -> Publication:
        ...
        if publication["source"] == PublicationSource.AUTHOR_PUBLICATION_ENTRY:
            ...
            res_dict = {}
            for year in years:
                res_dict[year] = nonzero_cites_per_year.get(year, 0)
