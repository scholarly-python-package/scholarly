...
class ProxyGenerator:
    ...
    def Luminati(self, usr, passwd, proxy_port):
        ...
        if usr is not None and passwd is not None and proxy_port is not None:
            ...
            self.logger.warning(
                "Not enough parameters were provided for the Luminati proxy. Reverting to a local connection."
            )
            return
    ...
    def SingleProxy(self, http=None, https=None):
        ...
        if proxy_works:
            ...
            self.logger.warning(
                "Unable to setup the proxy: http=%s https=%s. Reason unknown.",
                http,
                https,
            )
        return proxy_works
    ...
    def _use_proxy(self, http: str, https: str = None) -> bool:
        """Allows user to set their own proxy for the connection session. Sets the proxy if it works.
        ...
        if self.proxy_mode == ProxyMode.SCRAPERAPI:
            r = requests.get(
                "http://api.scraperapi.com/account", params={"api_key": self._API_KEY}
            ).json()
            if "error" in r:
                ...
                self.logger.info(
                    "Successful ScraperAPI requests %d / %d",
                    r["requestCount"],
                    r["requestLimit"],
                )
        else:
    ...
    @deprecated(
        version="1.5", reason="Tor methods are deprecated and are not actively tested."
    )
    def Tor_External(
        self, tor_sock_port: int, tor_control_port: int, tor_password: str
    ):
        ...
        if stem is None:
            raise RuntimeError(
                "Tor methods are not supported with basic version of the package. "
    ...
    def _get_chrome_webdriver(self):
        ...
        )  # Need to pre-load to set cookies later
    ...
    def ScraperAPI(self, API_KEY, country_code=None, premium=False, render=False):
        ...
        r = requests.get(
            "http://api.scraperapi.com/account", params={"api_key": API_KEY}
        ).json()
        if "error" in r:
            self.logger.warning(r["error"])
            return False
    ...
    def has_proxy(self) -> bool:
        """Checks if a proxy is available."""
